...
| 9 | **ClickUp משימות → Claude agent** (Dudi) | Webhook on the משימות list (own secret `CLICKUP_TASKS_WEBHOOK_SECRET`, events `taskCreated` + `taskCommentPosted`): a new task; a comment starting `Claude`/`קלוד` (the rest is a revision instruction, a bare one re-runs the task); the `הרץ שוב` button (`?action=clickup_to_claude`) | Claude does the marketing/content task as an agent: Dror's Google account (`src\lib\agent_tools.py`: search/read Drive, create a **branded** Doc, search/read Gmail, Gmail **drafts** only, never send) plus server-side web search/fetch. The task's `לקוח` relationship brings that client's details and questionnaire answers, and its Docs go to the client's `אסטרטגיה` folder. Posts the result as a comment. Runs through `tasks.dispatch`. Its comments are posted with Dror's token, so they are recognised by the 🤖/❌ prefix, never by author. Gmail needs the two extra delegation scopes in `docs\GOOGLE_SETUP.md`. |
