...
| 4b | **Signing follow-up** | Scheduled (daily) + a task per contract | `send_quote` opens a task on משימות (`src\lib\reminder_tasks.py`: `תזכורת חתימה: <client>`, due in 3 days, the email text in its description, no employee). `sign_reminders` sends it automatically on the due date if the client has not signed, with the text **as the description then reads**, the signing button and Dror's signature, then completes the task. Dror edits the text or date, or closes the task to cancel; signing closes it. No approval step, by Dror's choice. Contracts from before fall back to the fixed `sign_reminder` text at 3 days. |
...
| 7 | **Monthly campaign summary** | Scheduled (1st of month, `CampaignReportFunction`) / Button (`בנה דוח קמפיין`) | Pull the month's Meta Ads results, fill Dror's report template, add AI recommendations, save to Drive, and open a **review task** on משימות (`src\lib\review_tasks.py`: `דוח קמפיינים: <client> · <month>`, `לבדיקה של דרור`, linked by `לקוח`) with the PDF attached and the client's email as a Gmail draft behind a 📧 card: Dror's `שלח` sends it; a reply in the thread rebuilds the analysis with his note (`campaign_summary.revise`). No משימות list: emailed to Dror as before. The PDF renders in headless Chromium from a Lambda **layer** (`src\tools\publish_chromium_layer.py`), which `src\lib\pdf_chromium.py` inflates and drives over the DevTools protocol — no Playwright on Lambda (it renders only on a laptop, from `requirements-dev.txt`); prove it with a `{"check": "chromium"}` invoke. Emailing it still needs SMTP on the stack. |
| 8 | **Strategy bot** | Button (`בנה אסטרטגיה`, background task) / CLI | From the stored questionnaire answers + a live look at the digital presence (#3), Claude (adaptive thinking) writes the full strategy → Google Doc in `אסטרטגיה`, posted as a version on the review task `אסטרטגיה: <client>` on משימות; a reply revises it through #9 with the built-in `workers.STRATEGIST` (the same brief). No משימות list: email to Dror. Refuses without answers. |
| 9 | **ClickUp משימות → Claude agent** (Dudi) | Webhook on the משימות list (own secret `CLICKUP_TASKS_WEBHOOK_SECRET`, events `taskCreated` + `taskUpdated` + `taskCommentPosted`): a task whose **`עובד` Relationship field points at an agent in the `סוכנים` list** (`src\lib\workers.py`: the agent's task name is its name, its description its instructions, read every run; closed = off; a legacy dropdown with four built-ins still works; empty = a task for a person), at creation or when set later, once per (task, employee) (idempotency guard; switching employee hands it over again); it moves the task `in progress` → `לבדיקה של דרור` (Dror's inbox view), and the campaign manager gets a read-only `meta_ads_insights` tool bound to the client's ad account; a **reply in the thread under one of Claude's answers** (feedback, no keyword); a top-level `קלוד, ...` (feedback on the latest answer; bare `קלוד` or the `הרץ שוב` button = fresh run) | Claude does the marketing/content task as an agent: Dror's Google account (`src\lib\agent_tools.py`: read Drive and Gmail, Gmail **drafts** only, no Drive writes). **Actions need Dror's approval:** each Gmail draft the agent makes becomes a 📧 card in the answer's thread, and only Dror's own ClickUp user replying `שלח` there sends it (`agent_tools.send_draft`, the draft as it stands in Gmail); anyone else is refused plus server-side web search/fetch; the task's `לקוח` brings that client's details and questionnaire answers. **Every answer is a version** (`src\lib\task_docs.py`): a branded Google Doc `<task> - גרסה N` in the client's `משימות` subfolder (no client: `משימות Claude` in Dror's Drive), a PDF copy attached to the task (`claude-vN.pdf`, ASCII: ClickUp rejects Hebrew filenames), and a comment with the Doc link and the text or a preview. Revisions reply in the thread and read earlier versions back from their Docs. The webhook drops the bot's own comments by their 🤖/❌ prefix (they are posted with Dror's token); the job ignores comments that are not feedback before any model call. Runs through `tasks.dispatch`. |
...
| 11 | **Dashboard** | Always on (`DashboardFunction`, its own API) | Read-only web page over the run-log, grouped by subject, with links out; a **לידים** tab (`/leads`) of the Smoove signups, one row per phone (name, list, WhatsApp/call/copy), since they are not in ClickUp; **לקוחות** (`/clients`, and `/clients/<id>`: one client's card, with status, funnel stepper, their documents, questionnaire, details and activity) and **מסמכים** (`/documents`: every document the run-log links to, latest of each kind per client marked), both in `src\clients_pages.py`; **חוזים** (`/contracts`, `/contracts/<id>`: each client's contract state, the contract exactly as it will go out with a pre-send checklist, the signed document, and Dror's signature) in `src\contracts_pages.py`; from ClickUp (cached a minute), the run-log and the questionnaire store; plus the questionnaire admin (`/admin`: editor, preview, responses, links, CSV). `src\dashboard.py` renders; `src\dashboard_lambda.py` serves it behind API Gateway with the session in a signed cookie. Password = the `DashboardPassword` parameter. Also runs locally: `python -m src.dashboard`. |
| 12 | **Smoove → ManyChat** | Webhook (Smoove: lead) | Standalone AWS Lambda (`src\smoove_handler.py`). Smoove POSTs `{f_name, cellphone, msg}`; find/create the ManyChat contact by phone and trigger the **Flow** named by `msg` (`msg`→`MANYCHAT_FLOW_<MSG>`, unmapped is rejected). Logs the lead's name (Smoove's, else ManyChat's) and the list's name (its Flow's name in ManyChat); entries from before 2026-09-24 were filled in by `src\tools\backfill_leads.py`, which runs inside the Smoove Lambda (it holds the ManyChat key) on a direct invoke `{"task": "backfill_leads", "apply": true}`. Flow because the message is business-initiated → Meta-approved template only. |
