...
| 9 | **ClickUp משימות → Claude agent** (Dudi) | Webhook (a task *created* on the משימות list, its own secret `CLICKUP_TASKS_WEBHOOK_SECRET`) | Claude does the marketing/content task as an agent with tools on Dror's Google account (`src\lib\agent_tools.py`: search/read Drive, create a Doc, search/read Gmail, Gmail **drafts** only, never send), then posts the result as a comment. Async self-invoke (`{"claude_task_id"}`), no retries. Gmail needs the two extra delegation scopes in `docs\GOOGLE_SETUP.md`. |
